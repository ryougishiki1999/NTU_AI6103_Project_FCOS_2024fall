...
model_urls = {
    'vovnet27_slim': 'https://dl.dropbox.com/s/1lnzsgnixd8gjra/vovnet27slim_torchvision.pth?dl=1',
...
class VoVNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Initialize feature outputs for stage3, stage4, stage5
        stage3, stage4, stage5 = None, None, None

        # Pass through stages
        for i, name in enumerate(self.stage_names):
            x = getattr(self, name)(x)
            if name == 'stage3':  # Output for stage3
                stage3 = x
                # print(f"Stage3 output shape: {stage3.shape}")
            elif name == 'stage4':  # Output for stage4
                stage4 = x
                # print(f"Stage4 output shape: {stage4.shape}")
            elif name == 'stage5':  # Output for stage5
                stage5 = x
                # print(f"Stage5 output shape: {stage5.shape}")
    ...
    def freeze_stages(self, stage):
      ...
      for i in range(2, stage + 2):  # Stage starts from stage2
...
def vovnet39(pretrained=True, progress=True, **kwargs):
...
def vovnet27_slim(pretrained=True, progress=True, **kwargs):
    r"""Constructs a VoVNet-39 model as described in
    `"An Energy and GPU-Computation Efficient Backbone Networks"
    <https://arxiv.org/abs/1904.09730>`_.
    Args:
