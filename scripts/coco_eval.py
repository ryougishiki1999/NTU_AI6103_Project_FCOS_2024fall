...
if DefaultConfig.choosen_backbone is not DefaultConfig.Backbone.ResNet50:
    ...
    raise ValueError("CoCo Datset Evaluation only supports RestNet50 as backbone now!")
...
import torch
#ROOT = "/root/autodl-tmp/res/"
ROOT = DefaultConfig.coco_out_dir_path
from config import DefaultConfig
#valid_path = '/root/autodl-tmp/coco_2017/validation/data'
valid_path = DefaultConfig.coco_val_data_path
...
def evaluate_coco(generator, model, threshold=0.05):
    ...
    # write output
    #json.dump(results, open(f'{ROOT}coco_bbox_results.json', 'w'), indent=4)
    json.dump(results, open(os.path.join(ROOT, 'coco_bbox_results.json'), 'w'), indent=4)
    # json.dump(image_ids, open('{}_processed_image_ids.json'.format(generator.set_name), 'w'), indent=4)

    # load results in COCO evaluation tool
    coco_true = generator.coco
    #coco_pred = coco_true.loadRes(f'{ROOT}coco_bbox_results.json')
    coco_pred = coco_true.loadRes(os.path.join(ROOT, 'coco_bbox_results.json'))
    # run COCO evaluation
...
if __name__ == "__main__":
    ...
    #generator=COCOGenerator(valid_path,"/root/autodl-tmp/coco_2017/validation/labels.json")
    generator = COCOGenerator(
        valid_path,
        DefaultConfig.coco_val_label_path
    )
    model=FCOSDetector(mode="inference")
    model = torch.nn.DataParallel(model)
    model = model.cuda().eval()
    model.load_state_dict(torch.load(DefaultConfig.coco_train_eval_check_point_path, map_location=torch.device('cuda'))  ['model_state_dict']  )
