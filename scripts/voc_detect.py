...
if DefaultConfig.choosen_backbone is DefaultConfig.Backbone.ResNet50:
    ...
    raise ValueError("VOC Datset Detection only supports MobileNetV2 or VovNet27Slim or ResNet18 as backbone now!")
...
def show(model_path):
    ...
    import os
    #root="/content/drive/MyDrive/FCOS/test_images"
    root = DefaultConfig.voc_detect_images_dir_path
    ...
    for name in names:
        ...
        plt.savefig(os.path.join(DefaultConfig.voc_detect_out_images_dir_path, name), bbox_inches='tight', pad_inches=0.0)
        #plt.show()
        #plt.close()
